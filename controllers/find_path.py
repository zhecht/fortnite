...
def find_path_blueprint_route():
	...
	source = {
		"x": float(request.args["starting_x"]),
		"y": float(request.args["starting_y"])
	}
	...
	p,opt = findPath(chest_ids,neighbors,[],[],circle_x,circle_y,time_left,0)
	#print(path)


	return jsonify(returnArray(opt,all_chests))